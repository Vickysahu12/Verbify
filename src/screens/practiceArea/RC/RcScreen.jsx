/**
 * RcReadingScreen.jsx
 * Matches your stack: <Stack.Screen name="RcRead" component={RcReadingScreen} />
 * On finish → navigates to "RcResult" with params
 */
...
// ─── Session Data ─────────────────────────────────────────────────────────────
// TODO: Replace with API call using route.params.sessionId
const SESSION_DATA = {
  id: "session_rc_001",
  ...
  questions: [
    {
      ...
      difficulty: "Easy",
      analysis:
        "The passage clearly states that classical economists emphasized free markets, division of labor, and the invisible hand as self-regulating forces of the economy.",
    },
    {
      id: "q2",
      ...
      difficulty: "Medium",
      analysis:
        "The passage states that later schools believed markets were not always efficient, implying that real-world economic downturns exposed the limitations of the free-market model.",
    },
    {
      id: "q3",
      ...
      difficulty: "Hard",
      analysis:
        "The final sentence explicitly states that each school represented a response to the perceived failures of its predecessor, indicating an evolutionary, reactive relationship.",
    },
  ],
};
...
const OptionItem = React.memo(
  ({ option, index, label, isSelected, isCorrect, isWrong, onPress, disabled }) => {
      ...
      <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
        ...
        >
          <View
            style={[
              styles.optionLabel,
              { backgroundColor: borderColor === "transparent" ? "#E9F2EC" : borderColor },
            ]}
          >
...
const RcReadingScreen = () => {
  ...
  // TODO: fetch SESSION_DATA from API using sessionId

  const questions        = SESSION_DATA.questions;
  const passage          = SESSION_DATA.passage;
  const parsedPassage    = useMemo(() => parsePassage(passage.body), []);
  ...
  const submitSession = async (finalAnswers) => {
    ...
    // };
    // const res = await fetch("https://your-api.com/rc/submit", {
    //   method: "POST",
  ...
  // This is the KEY change — instead of showing result inline,
  // we navigate to "RcResult" and pass everything as route params
  ...
  }, [navigation, questions, sessionStartTime]);
  ...
  const handleNext = useCallback(async () => {
    if (isLastQuestion) {
      ...
      goToResult(answers); // ← navigate to RcResult screen
    } else {
      setShowAnalysis(false);
      setCurrentQ((q) => q + 1);
    }
  }, [isLastQuestion, answers, goToResult]);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtitle}>{SESSION_DATA.subject}</Text>
        </View>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={[styles.prevText, currentQ === 0 && { color: "#9CA3AF" }]}>
            ← Prev
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  iconBtn: {
    ...
    backgroundColor: "#E9F2EC", justifyContent: "center", alignItems: "center",
  },
  ...
  timerPill: {
    ...
    borderRadius: scale(20), borderWidth: 1.5,
    backgroundColor: "#FFFFFF", minWidth: scale(76), alignItems: "center",
  },
  ...
  progressBarBg: {
    height: scale(5), backgroundColor: "#DDE8E1",
    marginHorizontal: scale(16), borderRadius: scale(5),
  ...
  passageCard: {
    ...
    marginTop: scale(12), padding: scale(16), borderRadius: scale(20),
    ...Platform.select({
  ...
  passageLabelPill: {
    backgroundColor: "#E9F2EC", paddingHorizontal: scale(10),
    paddingVertical: scale(4), borderRadius: scale(20),
  },
  ...
  optionsWrap: { marginTop: scale(8) },

  option: {
    flexDirection: "row", alignItems: "center",
    marginHorizontal: scale(16), marginTop: scale(10),
    padding: scale(14), borderRadius: scale(16), gap: scale(12),
    ...Platform.select({
  ...
  optionLabelText: { fontSize: scale(12), fontWeight: "800" },
  ...
  analysisCard: {
    backgroundColor: "#FFFFFF", marginHorizontal: scale(16),
    marginTop: scale(16), padding: scale(16), borderRadius: scale(20),
    borderLeftWidth: 4, borderLeftColor: "#1F3B1F",
    ...Platform.select({
      ios: { shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 6 },
      android: { elevation: 3 },
    }),
  ...
  prevBtn: {
    ...
    borderRadius: scale(14), backgroundColor: "#FFFFFF",
  ...
  nextBtn: {
    paddingVertical: scale(13), paddingHorizontal: scale(22),
    borderRadius: scale(14), backgroundColor: "#1F3B1F",
    minWidth: scale(90), alignItems: "center",
  },
