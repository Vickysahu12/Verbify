...
const C = {
  ...
  bg: '#F8F7FC', surface: '#FFFFFF', border: '#E4E0F0', borderLight: '#F0EEF8',
  ...
  odd: '#EF4444', oddBg: '#FEF2F2',
};

export default function OddOneOutPracticeScreen({ navigation }) {
  ...
  const getOptionStyle = (qId, optId) => {
    ...
    if (optId === result.correct) return [s.option, s.optionOdd];   // correct = the odd one
  ...
  const getLetterBg = (qId, optId) => {
    const result = getResult(qId); const isSelected = answers[qId] === optId;
    if (!result) return isSelected ? C.accent : '#C4B8F0';
    if (optId === result.correct) return C.odd;
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      <View style={s.navbar}>
        ...
        <View style={s.navCenter} pointerEvents="none">
          <Text style={s.navTitle}>Odd One Out Practice</Text>
          <Text style={s.navSub}>FIND THE INTRUDER</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.infoBanner}>
          ...
          <Text style={s.infoBannerText}>
            Four sentences are given. Three form a coherent group. Tap the one that does NOT fit.
          </Text>
        </View>
            ...
            {questions.map((q, idx) => {
              ...
              return (
                <View key={q.id} style={s.questionBlock}>
                  ...
                  <Text style={s.optionsLabel}>Tap the odd sentence:</Text>

                  {/* Options — each IS a sentence */}
                  <Animated.View style={{ transform: [{ translateX: getShakeAnim(q.id) }] }}>
                    {(q.options ?? []).map(opt => {
                      const isOddAndCorrect = isCheckedQ && opt.id === result?.correct;
                      const isWrongPick     = isCheckedQ && opt.id === answers[q.id] && !isCorrect;
                        ...
                        <TouchableOpacity key={opt.id}
                          ...
                          <View style={{ flex: 1 }}>
                            <Text style={[s.optText, {
                              color: isOddAndCorrect ? C.odd : isWrongPick ? C.wrong : C.text
                            }]}>{opt.text}</Text>
                            {isOddAndCorrect && (
                              <View style={s.oddTag}>
                                <Text style={s.oddTagText}>🚫 ODD ONE OUT</Text>
                              </View>
                            )}
                            {isWrongPick && (
                              <View style={s.wrongTag}>
                                <Text style={s.wrongTagText}>✗ This fits the theme</Text>
                              </View>
                            )}
                          </View>
                        </TouchableOpacity>
                    ...
                    <View style={[s.feedbackBox, isCorrect ? s.feedbackCorrect : s.feedbackWrong]}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={s.feedbackTitle}>
                          {isCorrect ? 'Correct! You found the odd one.' : `Correct answer: ${result?.correct}`}
                        </Text>
...
const s = StyleSheet.create({
  ...
  statDiv: { width: 1, height: sc(24), backgroundColor: 'rgba(255,255,255,0.2)' },
  scrollContent: { paddingHorizontal: sc(16), paddingTop: sc(16), paddingBottom: sc(20) },
  infoBanner: { flexDirection: 'row', alignItems: 'center', gap: sc(10), backgroundColor: C.accentLight, borderRadius: sc(14), padding: sc(14), marginBottom: sc(16), borderWidth: 1, borderColor: '#D4C8F8' },
  ...
  option: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#F8F7FC', borderRadius: sc(13), padding: sc(13), marginBottom: sc(8), gap: sc(12), borderWidth: 1.5, borderColor: C.border },
  optionSelected: { borderColor: C.accent, backgroundColor: C.accentLight },
  optionOdd: { borderColor: C.odd, backgroundColor: C.oddBg },
  ...
  oddTag: { marginTop: sc(5), backgroundColor: '#FEE2E2', borderRadius: sc(5), paddingHorizontal: sc(7), paddingVertical: sc(2), alignSelf: 'flex-start' },
  oddTagText: { fontSize: sc(10), fontWeight: '800', color: '#991B1B' },
  wrongTag: { marginTop: sc(5), backgroundColor: '#DCFCE7', borderRadius: sc(5), paddingHorizontal: sc(7), paddingVertical: sc(2), alignSelf: 'flex-start' },
  wrongTagText: { fontSize: sc(10), fontWeight: '700', color: '#166534' },
