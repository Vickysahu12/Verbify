...
import { getExamData, getPassage, getQuestionNumbers } from "../MockPortal/data/examData";
...
const COLORS = {
  ...
  tabInactive:    "#9CA3AF",
  progressBg:     "#DDE1F5",
  timerWarning:   "#EF4444",   // timer turns red when < 5 min
};
...
/**
 * Build initial meta for every question in the list.
 * All start as NOT_VISITED; first question will be
 * marked VISITED_NOT_ANSWERED on mount via useEffect.
 */
...
/**
 * Returns human-readable label for question type badge
 */
...
export default function TestInterfaceScreen({ route }) {
  ...
  const examConfig = getExamData(mockId);

  // First unlocked section is the active one on load
  const [activeSectionId, setActiveSectionId] = useState(
    examConfig.sections.find((s) => !s.locked)?.id ?? examConfig.sections[0].id
  );
  ...
  const ALL_Q_NUMBERS  = getQuestionNumbers(QUESTIONS_DATA);

  // ── State ──
  const [currentQIndex,      setCurrentQIndex]      = useState(0);
  const [questionMeta,       setQuestionMeta]        = useState(() => buildInitialMeta(QUESTIONS_DATA));
  const [timeLeft,           setTimeLeft]            = useState(examConfig.totalSeconds);
  ...
  const isTimerLow = timeLeft < 5 * 60; // red when < 5 minutes
  ...
  const currentPassage = currentQuestion ? getPassage(currentQuestion.passageId) : null;
  ...
  const handleOptionSelect = (optionIndex) => {
    const meta        = questionMeta[currentQuestion.id];
    const alreadyMarked = meta?.status === Q_STATUS.MARKED_REVIEW;
    updateMeta(currentQuestion.id, {
      selected: optionIndex,
      status:   alreadyMarked ? Q_STATUS.ANSWERED_MARKED : Q_STATUS.ANSWERED,
    });
  };
  ...
  const handleTitaChange = (text) => {
    ...
    updateMeta(currentQuestion.id, {
      titaAnswer: text,
      status: hasAnswer
        ? (alreadyMarked ? Q_STATUS.ANSWERED_MARKED : Q_STATUS.ANSWERED)
  ...
  const handleSaveAndNext = () => {
    const meta = questionMeta[currentQuestion.id];
    // If nothing selected/typed, mark visited-not-answered
    const hasAnswer =
      meta?.selected !== null ||
      (meta?.titaAnswer && meta.titaAnswer.trim().length > 0);
    if (!hasAnswer) {
      updateMeta(currentQuestion.id, { status: Q_STATUS.VISITED_NOT_ANSWERED });
    }
    if (currentQIndex < QUESTIONS_DATA.length - 1) {
  ...
  const handleMarkForReview = () => {
    const meta = questionMeta[currentQuestion.id];
    const isAnswered =
      meta?.status === Q_STATUS.ANSWERED ||
      meta?.status === Q_STATUS.ANSWERED_MARKED;
    updateMeta(currentQuestion.id, {
  ...
  const handleClearResponse = () => {
    updateMeta(currentQuestion.id, {
      selected:   null,
      titaAnswer: "",
      status:     Q_STATUS.VISITED_NOT_ANSWERED,
    });
  };
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          <Text style={{ color: COLORS.subText, fontSize: scale(14) }}>
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.timerRow}>
        ...
        <View style={styles.timerLeft}>
          ...
          <Text style={[styles.timerColon, isTimerLow && { color: COLORS.timerWarning }]}>
            :
          </Text>
          <View style={[styles.timerBox, isTimerLow && styles.timerBoxWarning]}>
            <Text style={[styles.timerNum, isTimerLow && styles.timerNumWarning]}>
        ...
        <View style={styles.progressRight}>
          <View style={styles.progressLabelRow}>
            ...
            <Text style={styles.progressCount}>
              {answeredCount}/{activeSection?.totalQuestions ?? QUESTIONS_DATA.length}
            </Text>
          </View>
            ...
            <View
              style={[
                styles.progressBarFill,
                {
                  width: `${
                    (answeredCount /
                      (activeSection?.totalQuestions || QUESTIONS_DATA.length)) *
                    100
                  }%`,
                },
              ]}
      ...
      {/* ── SECTION TABS — dynamic from examConfig ── */}
              ...
              <View style={styles.tabInner}>
                <Text
                  style={[
                    ...
                    section.locked && styles.tabLabelLocked,
                  ]}
                >
                  {section.label}
                </Text>
      ...
      >
        {/* ── PASSAGE CARD — rendered only when question has a passageId ── */}
        {currentPassage && (
          <View style={styles.passageCard}>
            <View style={styles.passageHeader}>
              <View style={styles.passageBadge}>
                <Text style={styles.passageBadgeText}>{currentPassage.label}</Text>
              </View>
              <View style={styles.passageActions}>
                <TouchableOpacity
                  onPress={() => setPassageFontSize((f) => Math.min(f + scale(2), scale(22)))}
                  style={styles.fontBtn}
                >
                  <Text style={styles.fontBtnText}>A+</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => setPassageFontSize((f) => Math.max(f - scale(2), scale(12)))}
                  style={styles.fontBtn}
                >
            ...
            {currentPassage.content.map((para, idx) => (
              <Text
                key={idx}
                style={[styles.passageText, { fontSize: passageFontSize }]}
              >
                {para}
              </Text>
            ))}
        ...
        <View style={styles.questionCard}>
          {/* Header: Q-number badge + type label */}
          <View style={styles.questionHeader}>
            <View style={styles.qNumBadge}>
              <Text style={styles.qNumText}>Q{currentQuestion.number}</Text>
            </View>
            ...
            <View style={styles.marksBadge}>
              <Text style={styles.marksText}>
                ...
                {"  "}
                <Text style={{ color: COLORS.notAnswered }}>
                  {currentQuestion.marks?.incorrect ?? -1}
                </Text>
              </Text>
            </View>
          </View>
          ...
          {/* ── MCQ / Para Jumble / Para Summary / Odd-One-Out options ── */}
          {(currentQuestion.type === "mcq" ||
            currentQuestion.type === "para_jumble" ||
            currentQuestion.type === "para_summary" ||
          ...
          {currentQuestion.type === "tita" && (
            <View style={styles.titaContainer}>
              {currentQuestion.hint && (
                <Text style={styles.titaHint}>{currentQuestion.hint}</Text>
              )}
              <TextInput
          ...
          {/* Clear response */}
          {(currentMeta?.selected !== null ||
            (currentMeta?.titaAnswer && currentMeta.titaAnswer.length > 0)) && (
      ...
      <View style={styles.paletteStrip}>
        ...
        >
          {ALL_Q_NUMBERS.map((n) => {
            ...
            const bgColor   = getStatusColor(meta.status);
            const txtColor  = getStatusTextColor(meta.status);
            return (
              <TouchableOpacity
                key={n}
                style={[
                ...
                onPress={() => handleJumpToQuestion(n)}
                activeOpacity={0.75}
              >
                <Text
                  style={[
                    styles.paletteBubbleText,
                    { color: isCurrent ? COLORS.primary : txtColor },
...
const PaletteModal = ({
  visible, onClose, questionMeta, questionsData,
  allQNumbers, currentQNumber, onJump,
  answeredCount, notAnsweredCount, markedCount, notVisitedCount,
}) => (
            ...
            return (
              ...
              >
                <Text
                  style={[
                    modal.bubbleText,
                    { color: isCurrent ? COLORS.primary : getStatusTextColor(meta.status) },
                    isCurrent && { fontWeight: "800" },
                  ]}
                >
                  {n}
...
const SubmitModal = ({
  ...
  answeredCount, totalQuestions, notAnsweredCount, markedCount,
}) => (
    ...
    <View style={submitM.overlay}>
      <View style={submitM.card}>
        ...
        <Text style={submitM.warning}>
          ⚠️  Unanswered questions carry no marks. Marked-for-review questions are evaluated only if answered.
        </Text>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: Platform.OS === "android" ? scale(38) : scale(22),
    borderBottomWidth: 1,
  ...
  headerTitle: {
    ...
    color: COLORS.darkText, letterSpacing: 0.2,
  },
  headerSubject: {
    fontSize: scale(12), fontWeight: "700",
    color: COLORS.primary, marginTop: 1, letterSpacing: 0.5,
  },
  submitBtn: {
    backgroundColor: COLORS.primaryLight,
    paddingHorizontal: scale(14), paddingVertical: scale(8),
    borderRadius: scale(20), borderWidth: 1.5, borderColor: COLORS.primary,
  },
  ...
  timerBox: {
    backgroundColor: COLORS.timerBox, borderRadius: scale(8),
    paddingHorizontal: scale(10), paddingVertical: scale(6),
    minWidth: scale(42), alignItems: "center",
  },
  timerBoxWarning: { backgroundColor: "#FEE2E2" },
  timerNum: {
    fontSize: scale(20), fontWeight: "800",
    color: COLORS.darkText, letterSpacing: 1,
  },
  timerNumWarning: { color: COLORS.timerWarning },
  timerColon: {
    fontSize: scale(20), fontWeight: "800",
    color: COLORS.darkText, marginHorizontal: scale(4),
  },
  remainingLabel: {
    fontSize: scale(10), fontWeight: "600",
    color: COLORS.subText, marginLeft: scale(8), letterSpacing: 0.8,
  },
  ...
  progressBarBg: {
    width: scale(120), height: scale(5),
    backgroundColor: COLORS.progressBg, borderRadius: 10, overflow: "hidden",
  },
  ...
  tabsRow: {
    backgroundColor: COLORS.white,
    flexDirection: "row", paddingHorizontal: scale(16),
  },
  tab: { marginRight: scale(24), paddingTop: scale(8) },
  tabInner: {
    flexDirection: "row", alignItems: "center",
    gap: 4, paddingBottom: scale(8),
  },
  tabLabel: {
    fontSize: scale(13), fontWeight: "700",
    color: COLORS.tabInactive, letterSpacing: 0.5,
  },
  tabLabelActive:  { color: COLORS.tabActive },
  tabLabelLocked:  { color: COLORS.notVisited },
  lockIcon:        { fontSize: scale(10) },
  tabUnderline: {
    height: 2.5, backgroundColor: COLORS.primary,
  ...
  passageCard: {
    backgroundColor: COLORS.white,
    borderRadius: scale(14), padding: scale(16),
  ...
  passageText: {
    color: COLORS.darkText, lineHeight: scale(24), marginBottom: scale(14),
  ...
  questionType: {
    flex: 1, fontSize: scale(11),
    color: COLORS.subText, fontWeight: "600", letterSpacing: 0.3,
  },
  marksBadge: {
    backgroundColor: COLORS.background,
  ...
  marksText: {
    fontSize: scale(11), fontWeight: "700", color: COLORS.answered,
  },
  questionText: {
    fontSize: scale(14.5), color: COLORS.darkText,
    lineHeight: scale(22), fontWeight: "500", marginBottom: scale(18),
  },
  ...
  optionRow: {
    ...
    borderWidth: 1.5, borderColor: "transparent", gap: scale(12),
  },
  optionRowSelected: {
  ...
  optionCircle: {
    ...
    justifyContent: "center", alignItems: "center",
    marginTop: scale(1), flexShrink: 0,
  },
  optionCircleSelected: { borderColor: COLORS.primary },
  optionCircleDot: {
    width: scale(9), height: scale(9),
    borderRadius: scale(4.5), backgroundColor: COLORS.primary,
  },
  optionText: {
    flex: 1, fontSize: scale(14),
    color: COLORS.darkText, lineHeight: scale(20),
  },
  ...
  titaHint: {
    fontSize: scale(12), color: COLORS.subText,
    marginBottom: scale(8), fontStyle: "italic",
  },
  titaInput: {
    backgroundColor: COLORS.background,
    borderRadius: scale(10), padding: scale(14),
  ...
  // Clear
  clearBtn: {
    marginTop: scale(14), alignSelf: "flex-start",
    paddingHorizontal: scale(12), paddingVertical: scale(6),
    borderRadius: scale(8), borderWidth: 1, borderColor: COLORS.notAnswered,
  },
  ...
  paletteStrip: {
    backgroundColor: COLORS.white,
    paddingTop: scale(10), paddingBottom: scale(6),
    borderTopWidth: 1, borderTopColor: COLORS.border,
  },
  paletteTitleRow: {
    flexDirection: "row", justifyContent: "space-between",
    alignItems: "center", paddingHorizontal: scale(16), marginBottom: scale(8),
  },
  paletteTitleText: {
    fontSize: scale(10), fontWeight: "700",
    color: COLORS.subText, letterSpacing: 1,
  },
  ...
  // Bottom bar
  bottomBar: {
    flexDirection: "row", padding: scale(12),
    paddingBottom: Platform.OS === "ios" ? scale(4) : scale(12),
    backgroundColor: COLORS.white,
    borderTopWidth: 1, borderTopColor: COLORS.border, gap: scale(12),
  },
  reviewBtn: {
    ...
    backgroundColor: COLORS.white,
    borderWidth: 1.5, borderColor: COLORS.border, gap: scale(6),
  },
  ...
  saveNextBtnText: {
    fontSize: scale(15), fontWeight: "800",
...
const modal = StyleSheet.create({
  ...
  sheet: {
    ...
    paddingHorizontal: scale(20), paddingBottom: scale(32),
    maxHeight: height * 0.8,
  },
  handle: {
    width: scale(40), height: 4, backgroundColor: COLORS.border,
    borderRadius: 2, alignSelf: "center", marginVertical: scale(12),
  },
  title: {
    fontSize: scale(16), fontWeight: "800",
    color: COLORS.darkText, marginBottom: scale(14),
  },
  ...
  bubble: {
    ...
    justifyContent: "center", alignItems: "center", margin: scale(4),
  },
  bubbleCurrent: {
  ...
  closeBtn: {
    ...
    paddingVertical: scale(14), alignItems: "center", marginTop: scale(8),
  },
...
const submitM = StyleSheet.create({
  ...
  card: {
    backgroundColor: COLORS.white, borderRadius: scale(20),
    padding: scale(24), width: "100%",
  },
  title: {
    fontSize: scale(18), fontWeight: "800",
    color: COLORS.darkText, marginBottom: scale(6),
  },
  ...
  warning: {
    fontSize: scale(12), color: COLORS.subText, lineHeight: scale(18),
    backgroundColor: "#FFFBEB", padding: scale(12), borderRadius: scale(10),
    borderLeftWidth: 3, borderLeftColor: "#F59E0B", marginBottom: scale(20),
  },
  actions: { flexDirection: "row", gap: scale(12) },
  cancelBtn: {
    flex: 1, paddingVertical: scale(14), borderRadius: scale(12),
    borderWidth: 1.5, borderColor: COLORS.border, alignItems: "center",
  },
  cancelText: { fontSize: scale(14), fontWeight: "700", color: COLORS.darkText },
  confirmBtn: {
    flex: 1.5, paddingVertical: scale(14),
    borderRadius: scale(12), backgroundColor: COLORS.primary, alignItems: "center",
  },
