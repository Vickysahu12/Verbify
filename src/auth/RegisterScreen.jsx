...
const RegisterScreen = ({ navigation }) => {
  ...
  const validateForm = () => {
    ...
    else if (!/^[0-9]{10}$/.test(formData.phone.replace(/\s/g, ""))) newErrors.phone = "Enter valid 10-digit phone number";
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.form}>
            <View>
              <TextInput
                ...
                onChangeText={(text) => { setFormData({ ...formData, fullName: text }); if (errors.fullName) setErrors({ ...errors, fullName: null }); }}
            ...
            <View>
              <TextInput
                ...
                onChangeText={(text) => { setFormData({ ...formData, phone: text }); if (errors.phone) setErrors({ ...errors, phone: null }); }}
                maxLength={10}
              />
            ...
            <View>
              <TextInput
                ...
                onChangeText={(text) => { setFormData({ ...formData, email: text.toLowerCase() }); if (errors.email) setErrors({ ...errors, email: null }); }}
              />
            ...
            <View>
              <View style={[styles.passwordWrapper, errors.password && styles.inputError]}>
                <TextInput
                  ...
                  secureTextEntry={!showPassword} style={styles.passwordInput}
                  value={formData.password}
                  onChangeText={(text) => { setFormData({ ...formData, password: text }); if (errors.password) setErrors({ ...errors, password: null }); }}
                />
            ...
            <View>
              <View style={[styles.passwordWrapper, errors.confirmPassword && styles.inputError]}>
                <TextInput
                  placeholder="Confirm Password" placeholderTextColor="#6B7280"
                  secureTextEntry={!showConfirmPassword} style={styles.passwordInput}
                  value={formData.confirmPassword}
                  onChangeText={(text) => { setFormData({ ...formData, confirmPassword: text }); if (errors.confirmPassword) setErrors({ ...errors, confirmPassword: null }); }}
                />
          ...
          <View style={styles.termsContainer}>
            <TouchableOpacity onPress={() => setAgreedToTerms(!agreedToTerms)} style={styles.checkbox}>
              <Ionicons name={agreedToTerms ? "checkbox" : "square-outline"} size={22} color={agreedToTerms ? "#1F3B1F" : "#6B7280"} />
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  progressContainer: { height: 6, backgroundColor: "#E5E7EB", borderRadius: 10, marginTop: 50, overflow: "hidden" },
  progressActive: { width: "35%", height: "100%", backgroundColor: "#1F3B1F", borderRadius: 10 },
  header: { alignItems: "center", marginTop: 16 },
  title: { fontSize: 20, fontWeight: "600", color: "#111827" },
  brand: { fontSize: 45, fontWeight: "800", color: "#1F3B1F", marginTop: 4 },
  form: { marginTop: 60 },
  input: { backgroundColor: "#E5EFE5", borderRadius: 30, paddingHorizontal: 20, paddingVertical: 14, fontSize: 14, color: "#111827", marginTop: 12 },
  inputError: { borderWidth: 1, borderColor: "#EF4444" },
  errorText: { color: "#EF4444", fontSize: 12, marginTop: 4, marginLeft: 20, fontWeight: "500" },
  passwordWrapper: { backgroundColor: "#E5EFE5", borderRadius: 30, paddingHorizontal: 20, paddingVertical: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 12 },
  passwordInput: { flex: 1, fontSize: 14, color: "#111827" },
  ...
  termsLink: { color: '#1F3B1F', fontWeight: '600' },
  bottomSection: { paddingBottom: 30 },
  button: { backgroundColor: "#1F3B1F", borderRadius: 30, paddingVertical: 15, alignItems: "center" },
  buttonDisabled: { opacity: 0.6 },
  buttonText: { color: "#FFFFFF", fontSize: 16, fontWeight: "600" },
  footer: { flexDirection: "row", justifyContent: "center", marginTop: 16 },
  footerText: { color: "#6B7280", fontSize: 13 },
  signIn: { color: "#4CAF50", fontWeight: "600", fontSize: 13 },
});